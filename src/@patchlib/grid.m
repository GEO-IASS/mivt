...
%       should be the same length as volSize. By default, the patches are assumed to be sliding -
%       that is, patches will have an overlap of (patchSize - 1). See below for specifying patch
%       overlaps.
%
...
%       will be cropped to the maximum size that fits the patch grid. For example, a [6x6] volume
%       with a patchsize of [3x3] and overlap of 1 will be cropped to [5x5] volume. Overlap should
%       be < patchSize. patchOverlap should be a scalar (applied to all dim) or the same length as
%       patchSize.
%
...
%       'sliding'.
%
%   idx = grid(..., patchOverlap/kind, startSub) - start the indexing at a particular location [1 x
%       nDims]. This essentially means that the volume will be cropped starting at that location.
