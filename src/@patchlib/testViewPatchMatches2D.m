function testViewPatchMatches2D()
    ...
    patchlib.viewPatchMatches2D(origPatch, matches);
    
    % test simple original and matches, with a different intensity scale, and segmentations
    patchlib.viewPatchMatches2D(origPatch*100, matches*100, seg, 'caxis', {[0 100], [0, 100], [0, 1]});
    
    % test original, matches, label equivalent and a second label 
    patchlib.viewPatchMatches2D(origPatch, matches, seg, seg .* (rand(size(seg)) < 0.5));
    ...
    patchlib.viewPatchMatches2D(origPatches, matchescell, segcell);
